...
타임라인에서 제외할 항목:
- "확인해보겠다", "보겠습니다" 등 단순 확인/응답 메시지
- 링크만 공유하고 실질적 내용이 없는 메시지
- 별도 채널 이동 요청 등 절차적 메시지
- 동일한 사실을 중복 언급하는 경우 첫 번째만 유지`;
...
타임라인에서 제외할 항목 (가치 없는 메시지):
- channel_join, channel_leave 등 시스템 메시지
- "확인해보겠다", "보겠습니다", "알겠습니다" 등 단순 확인/응답 메시지
- 링크만 공유하고 실질적 내용이 없는 메시지 (예: APM 링크 공유, 트레이스 링크 공유)
- 별도 채널로 이동 요청 등 절차적 메시지
- 동일한 사실을 중복 언급하는 경우 첫 번째 항목만 유지
- 장애와 직접 관련 없는 메시지`;
